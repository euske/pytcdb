...
def main(argv):
  ...
  # cdb
  if cmd == 'cmake':
    CDBMaker(dbname).txt2cdb(fileinput.input(args)).finish()
  elif cmd == 'cget':
    print CDBReader(dbname).lookup(args)
  elif cmd == 'cdump':
  ...
  # tcdb
    ...
    TCDBMaker(dbname).txt2tcdb(fileinput.input(args)).finish()
  elif cmd == 'tget':
